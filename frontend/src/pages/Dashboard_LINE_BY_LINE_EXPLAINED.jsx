/*
📌 FILE: Dashboard.jsx (versi dijelaskan baris per baris)
🧠 Fungsi file ini: Dashboard user setelah login: sidebar (logo, search, nav Home/My Report/All Items, user info), main (header + link Report Lost/Found, section Lost Items, Found Items, Resolved Items); fetch GET /items; filter lost/found/resolved + search; tampil CardItem.
🔄 Alur singkat: useLocation; state items, loading, user, searchQuery, sidebarOpen; useEffect load user dari localStorage, useEffect fetchItems(); lostItems/foundItems/resolvedItems = filter; filterItems(itemList) pakai searchQuery; return layout sidebar + overlay + main (header + 3 section).
📦 Analogi: Papan utama setelah masuk: menu samping (cari, Home, Laporan Saya, Semua Barang), area tengah (sapaan + tombol lapor + tiga kelompok kartu: hilang, ditemukan, selesai).
*/
...
/*
🔄 Alur eksekusi: useLocation; state; useEffect user dari localStorage; useEffect fetchItems (GET /items); lostItems/foundItems/resolvedItems; filterItems; return div (aside sidebar + conditional overlay + main header + main 3 section Lost/Found/Resolved dengan loading/grid/empty).
🧠 Ringkasan: Dashboard user: sidebar dengan search dan nav; konten: Lost, Found, Resolved; data dari /items; filter search by name/description/location.
📘 Glosarium: useLocation, pathname, filter, map, key, showDelete=false, resolved_at.
⚠️ Kesalahan: Lupa key di map → React warning; fetchItems tanpa token → pastikan route protected dan api pakai interceptor token.
*/
