...
🧠 Fungsi: Halaman utama (landing): hero, search bar, tiga section (Lost, Found, Resolved). Fetch GET /items sekali saat mount; filter client-side lost/found/resolved; filterItems by searchQuery. Tampil CardItem; loading spinner; empty state.
🔄 Alur: Mount → useEffect fetchItems → set items, loading false. lostItems/foundItems/resolvedItems = filter dari items. filterItems(itemList) pakai searchQuery. Render hero, search input, tiga section dengan map CardItem.
📦 Analogi: Seperti papan pengumuman utama: judul, kotak cari, lalu tiga kelompok pengumuman (hilang, temuan, selesai).
*/
...
/*
---------- RINGKASAN: useState items, loading, searchQuery. useEffect fetchItems. lostItems/foundItems/resolvedItems dari filter. filterItems by searchQuery. Render hero, search, tiga section (loading/empty/grid CardItem).
---------- ISTILAH: useEffect([]) = jalankan sekali saat mount; client-side filter = filter di frontend dengan .filter(); controlled input (value + onChange).
---------- KESALAHAN PEMULA: Lupa key={item.id} di map → warning React. Fetch di render tanpa useEffect → loop tak berujung. Salah dependency useEffect (misal [items]) → fetch berulang.
*/
