...
🧠 Fungsi: Dashboard admin: SidebarAdmin (tab items/users), fetch admin/items, admin/users, admin/statistics. Tab items: statistik, filter status, daftar item dengan tombol Approve/Reject/Edit/Delete/Resolve; modal resolve dan modal edit. Tab users: daftar user, tombol delete. Proteksi: useEffect cek role admin, bukan admin redirect dashboard.
🔄 Alur: useEffect cek user role; fetchData (Promise.all admin API). handleValidationStatus, handleMarkAsResolved, handleOpenEditModal/handleUpdateItem, handleDeleteItem/User. State: items, users, statistics, activeTab, filterStatus, modal state, editForm, dll.
📦 Analogi: Ruang admin: pilih "Kelola Laporan" atau "Kelola User", lihat statistik, approve/tolak/edit/hapus/resolve.
*/
...
/*
---------- RINGKASAN: Proteksi admin; fetchData (items, users, statistics); activeTab items/users; validasi, resolve, edit (FormData tanpa set Content-Type), delete item/user; modal resolve & edit.
---------- ISTILAH: Promise.all; controlled tab (activeTab/setActiveTab); modal (state show/hide); window.confirm.
---------- KESALAHAN PEMULA: Set Content-Type multipart/form-data manual saat edit item dengan gambar → sama seperti ReportLost, harus dihapus. Lupa cek role di useEffect → user biasa bisa buka /admin sebentar sebelum redirect.
*/
