...
🧠 Fungsi: Form lapor barang hilang: name, description, location, date_occured, file gambar (opsional). Submit: FormData append field + status "lost" + file "image"; api.post("/items/lost", data). Jangan set Content-Type manual (biarkan Axios set boundary). Sukses toast, navigate dashboard.
🔄 Alur: State form, file, imagePreview, error, loading. handleImageChange: validasi 5MB, setFile, preview. submit: FormData, api.post → toast, navigate atau setError.
📦 Analogi: Formulir pengaduan "barang saya hilang" dengan lampiran foto.
*/
...
/*
---------- RINGKASAN: FormData + append status "lost" + image; api.post("/items/lost", data). Jangan set header Content-Type manual.
---------- ISTILAH: FormData; FileReader.readAsDataURL (preview); multipart/form-data (boundary otomatis oleh Axios).
---------- KESALAHAN PEMULA: Set headers: { "Content-Type": "multipart/form-data" } → backend tidak dapat file (boundary hilang).
*/
