/*
📌 FILE: AdminDashboard.jsx (versi dijelaskan baris per baris)
🧠 Fungsi file ini: Dashboard admin: SidebarAdmin (tab items/users), statistik, tab Items (filter all/pending/approved/rejected, grid CardItem + tombol Approve/Reject, Mark Resolved, Edit, Delete), tab Users (tabel user + Delete); modal Resolve dan Edit; fetch /admin/items, /admin/users, /admin/statistics; proteksi role admin.
🔄 Alur singkat: useEffect cek localStorage user role → bukan admin redirect; useEffect fetchData (items, users, statistics); handler validasi, resolve, edit, delete item/user; filteredItems by filterStatus; return layout SidebarAdmin + header + main (statistics + tab items/users) + 2 modal.
📦 Analogi: Ruang admin: menu Manage Reports / Manage Users, kartu statistik, daftar laporan (filter, approve/reject, selesai, edit, hapus), daftar user (hapus); dua pop-up: tandai selesai dan edit laporan.
*/
...
/*
🔄 Alur eksekusi: useEffect cek user role (bukan admin → redirect); useEffect fetchData (Promise.all items, users, statistics); handler validasi/resolve/edit/delete item dan delete user; filteredItems; return SidebarAdmin + header + main (statistics + tab items dengan filter + grid CardItem + tombol aksi, tab users dengan tabel) + modal Resolve + modal Edit.
🧠 Ringkasan: Dashboard admin: statistik, Manage Reports (filter, approve/reject, resolve, edit, delete), Manage Users (tabel, delete); modal untuk resolve dan edit report.
📘 Glosarium: activeTab, filterStatus, validation_status, resolved_at, FormData, window.confirm, disabled={user.role==="admin"}.
⚠️ Kesalahan: Hapus user admin → tombol Delete disabled untuk role admin; jangan lupa fetchData() setelah mutasi agar list ter-update.
*/
